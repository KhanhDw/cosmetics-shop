...
const SystemConfigPage: React.FC = () => {
  const [activeTab, setActiveTab] = useState<
    "store" | "shipping" | "payment" | "email" | "sms"
  >("store");
          ...
          <div className="bg-[var(--admin-card-bg)] p-6 rounded-xl border border-[var(--admin-border)]">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-[var(--admin-border)]">
                <thead>
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-medium text-[var(--admin-text-secondary)] uppercase tracking-wider">
                      Zone Name
                    </th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-[var(--admin-text-secondary)] uppercase tracking-wider">
                      Countries
                    ...
                    <th className="px-4 py-3 text-right text-xs font-medium text-[var(--admin-text-secondary)] uppercase tracking-wider">
                      Actions
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[var(--admin-border)]">
                  {config.shippingSettings.shippingZones.map((zone, index) => (
                    <tr key={index}>
                      <td className="px-4 py-4 whitespace-nowrap text-sm text-[var(--admin-text-primary)]">
                        {zone.name}
                      </td>
                      <td className="px-4 py-4 text-sm text-[var(--admin-text-primary)]">
                        {zone.countries.join(", ")}
                      </td>
                      <td className="px-4 py-4 whitespace-nowrap text-sm text-[var(--admin-text-primary)]">
                        ${zone.shippingFee}
                      ...
                      <td className="px-4 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <button className="text-[var(--admin-text-secondary)] hover:text-[var(--admin-text-accent)]">
                          Edit
                        </button>
                      </td>
                    </tr>
          ...
          <div className="bg-[var(--card-bg)] dark:bg-[var(--card-bg)] p-6 rounded-xl border border-[var(--border)] dark:border-[var(--border)]">
            ...
            <div className="mb-6">
              ...
              <div className="flex flex-wrap gap-4">
                {(
                  ["cod", "card", "bank_transfer", "vnpay", "momo"] as const
                ).map((gateway) => (
                  <label
                    key={gateway}
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={config.paymentSettings.enabledGateways.includes(
                        gateway
                      )}
                      onChange={(e) => {
                        if (e.target.checked) {
                          setConfig((prev) => ({
                            ...
                            paymentSettings: {
                              ...
                              enabledGateways: [
                                ...prev.paymentSettings.enabledGateways,
                                gateway,
                              ],
                            },
                          }));
                          ...
                          setConfig((prev) => ({
                            ...
                            paymentSettings: {
                              ...prev.paymentSettings,
                              enabledGateways:
                                prev.paymentSettings.enabledGateways.filter(
                                  (g) => g !== gateway
                                ),
                            },
                          }));
            ...
            {config.paymentSettings.vnpay &&
              config.paymentSettings.enabledGateways.includes("vnpay") && (
                <div className="border border-[var(--border)] dark:border-[var(--border)] rounded-lg p-4 mb-4">
                  <h3 className="text-lg font-medium text-[var(--text-primary)] dark:text-[var(--text-primary)] mb-3">
                    VNPay Settings
                  </h3>
